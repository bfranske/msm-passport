...
def index():
    ...
    if 'pdf' in request.args:
        return Response(weasyprint.HTML(string=myHTML).write_pdf(stylesheets=[weasyprint.CSS(string=weasyCSS)]), mimetype="application/pdf")
    else:
        return Response(myHTML, mimetype="text/html")
